...
def main(name='breakout', test=False, chkpt=None, device="cuda"):
    if not test:
        ...
        while True:
            ...
            if do_boltzman_exploration:
                ...
                if random() < eps:
                    ...
                    action = m(torch.Tensor(last_observation).unsqueeze(0).to(device)).max(-1)[-1].item()  # your agent here
            
            if test and initiate_sequence > 0:
...
if __name__ == "__main__":
    ...
    main(test=True, chkpt="models/8416667.pth")